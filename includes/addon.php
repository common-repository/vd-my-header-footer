...
class vD_My_Header_Footer {
  ...
  public static function setup() {
    ...
    if (!$headerPostId) {
      $headerPostId = vD_Post::create(vD_Post::VD_HEADER_POST, vD_Post::VD_HEADER_POST_TITLE, file_get_contents(path_join($dir, 'partials/main-header-tmpl.html')), 'wp_template', 'vd_tmpl_header', 'vd_posts', 'draft');
    }
    if (!$footerPostId) {
      $footerPostId = vD_Post::create(vD_Post::VD_FOOTER_POST, vD_Post::VD_FOOTER_POST_TITLE, file_get_contents(path_join($dir, 'partials/main-footer-tmpl.html')), 'wp_template', 'vd_tmpl_footer', 'vd_posts', 'draft');
    }
    if (!$themeStylePrivId) {
  ...
  public static function on_activate() {
    ...
    if (is_multisite()) {
      ...
      //   deactivate_plugins('vd-my-header-footer/vd_my_header_footer.php');
  ...
  public static function show_pro_page() {
    // include_once('partials/vd-mhf-pro-page.php');
    //exit( wp_redirect( 'https://www.visualdesigner.io/my-header-footer' ) );
  }
  ...
  public static function form_post_vd_wizard_submit() {
    ...
    if ($customFooter && isset($footerTemplate)) {
      $footerHtml = self::get_mhf_template($footerTemplate, path_join($dir, 'partials/main-footer-tmpl.html'));
    ...
    if ($customHeader && isset($headerTemplate)) {
      $headerHtml = self::get_mhf_template($headerTemplate, path_join($dir, 'partials/main-header-tmpl.html'));
      vD_Post::save($header->ID, $headerHtml, ["post_status" => "draft", "overwrite_all" => true]);
      $result['editUrl'] = $headerUrl;
    }
    //if (isset($hideElementsCSS)) {
  ...
  private static function get_mhf_template($tmpl_id, $fallback) {
    $tmplFile = path_join(plugin_dir_path(__FILE__), "partials/templates/$tmpl_id.html");
  ...
  public static function notices() {
    // if (get_option('VD_NOTIF_MHF_REMINDER', 'false') == 'true') {
      // show notification to rehide headers and footers for newly activated theme
      // include_once 'partials/vd-notification.php';
    // }
    ...
    // $hoursSinceDismiss3 = round(abs(get_option('VD_MHF_NAG_3_DISMISS_TIME', $utcTime - 1000000) - $utcTime) / ( 60 * 60 ), 2);

    // if ($hoursSinceDismiss3 > 72) {
    //   include_once 'partials/vd-notif-survey.php';
    // }
  ...
  public static function save_wp_template_post($content, $post_id, $post_title, $data) {
    ...
    if (isset($data["mhf_style"])) {
        ...
        // $fHandle = fopen(path_join(plugin_dir_path( __FILE__ ), 'css/mhf-style.css'), 'w');
        // fwrite($fHandle, 'body { background-color: blue !important; }');
        // fclose($fHandle);
    }
  ...
  public static function backwards_compat() {
    ...
    //if (version_compare(VD_ADDON_MHF_VERSION, '1.11') <= 0) {
